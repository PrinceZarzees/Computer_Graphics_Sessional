...
intersection_point_property find_diffuse_specular_component(vector<light_source> ls_list, objects objs, intersection_point_property ip, point camera)
{
    ...
    // if (lambert ==0 && phong ==0 && ip.P.x>=-30 && ip.P.x<=0 && ip.P.y>=-30 && ip.P.y<=0 && ip.P.z>=-0.0001 && ip.P.z<=0.0001)
    // {
    //     cout<<ip.P.x<<" "<<ip.P.y<<" "<<ip.P.z<<ip.red<<" "<<ip.green<<" "<<ip.blue<<endl;
    // }
    return ip;
}
...
void create_pointBuffer(double near_plane_d, double far_plane_d, double fovy, double aspect_ratio, Vector l, Vector r, Vector u, point camera, int image_width, int image_height)
{
    ...
    objs.add_cube(cube(point(-100, -100, 10), 40));
    objs.cubes[0].set_color(0, 0.5, 1);
    objs.cubes[0].set_color_component(0.15, 0.1, 0.4, 0.45, 10);
    objs.add_sphere(Sphere(point(20, 20, 20), 20));
    objs.spheres[0].set_color(0.25, 0.3, 1);
    objs.spheres[0].set_color_component(0.05, 0.1, 0.4, 0.55, 30);
    objs.add_sphere(Sphere(point(-20, -20, 20), 15));
    objs.spheres[1].set_color(1, 0, 1);
    objs.spheres[1].set_color_component(0.2, 0.3, 0.1, 0.3, 30);
    objs.add_pyramid(pyramid(point(-40 + 30 / 2.0, 0 + 30 / 2.0, 5), 30, 40));
    objs.pyramids[0].set_color(1, 0, 0);
    objs.pyramids[0].set_color_component(0.4, 0.2, 0.0, 0.4, 1);
    objs.add_checkerboard(checkerboard(50));
    objs.cb.set_color_component(0.1, 0.3, 0.6);

    vector<light_source> ls_list;
    ls_list.push_back(light_source(point(70, 70, 100), point(), 0, 0.000002, true));
    ls_list.push_back(light_source(point(-70, 70, 70), point(-10, 10, 10), 30 * acos(-1) / 180, 0.0000002, false));
    bitmap_image *image;
    ...
    for (int i = 0; i < image_height; i++)
    {
        for (int j = 0; j < image_width; j++)
        {
            ...
            ray = ray.normalize();
            intersection_point_property ip = find_color(ray, objs, pointBuffer[j][i], ls_list,5);
            if (ip.t != -10)
            {
