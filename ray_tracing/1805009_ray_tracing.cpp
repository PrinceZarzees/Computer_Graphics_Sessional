...
void ray_trace(double near_plane_d, double far_plane_d, double fovy, double aspect_ratio, Vector l, Vector r, Vector u, point camera, int image_width, int image_height, int recursion_level, objects objs, vector<light_source> ls_list)
{
    ...
    std::cout << "Point buffer generation done" << endl;
    ...
    for (int i = 0; i < image_height; i++)
    {
        ...
        int temp1=(int)((i+1)/(double)image_height*100);
        if (temp1==temp2)
        {cout<<"Rendering "<<temp1<<"\% complete"<<endl;
        temp2+=10;
        }
