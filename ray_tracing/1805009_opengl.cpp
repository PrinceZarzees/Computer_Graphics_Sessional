...
#include <vector>
#include <iostream>
#include "1805009_ray_tracing.cpp"
...
void keyboardListener(unsigned char key, int x, int y)
{
    ...
    {
    case '0':
        ray_trace(near_plane_d, far_plane_d, fovy, aspect_ratio, l, r, u, pos, image_resolution,image_resolution,recursion_level, objs, ls_list);
        break;
    case '1':
...
void displayFunc()
{
    ...
    //draw a cone in place of light source
    for(int i=0;i<ls_list.size();i++)
    {
        if (ls_list[i].normal==false)
            {
                GLdouble cone_height = 30, cone_radius = 5;
                GLdouble cone_x = ls_list[i].p.x, cone_y = ls_list[i].p.y, cone_z = ls_list[i].p.z;
                GLdouble cone_axis_x = ls_list[i].direction.x-cone_x, cone_axis_y = ls_list[i].direction.y-cone_y, cone_axis_z = ls_list[i].direction.z-cone_z;
                GLdouble cone_angle = ls_list[i].angle*180/M_PI;
                glColor3f(0.7f, 0.7f, 0.7f);
                glPushMatrix();
                glTranslatef(cone_x, cone_y, cone_z);
                Vector v=Vector(cone_axis_x,cone_axis_y,cone_axis_z);
                Vector z=Vector(0,0,1);
                glRotatef(acos(v.dot(z)/v.length())*180/M_PI, -cone_axis_y, cone_axis_x,0);
                glutSolidCone(cone_radius,cone_height,100,1);
                glPopMatrix();
            }
        else
        {
            ...
            glTranslatef(ls_list[i].p.x, ls_list[i].p.y, ls_list[i].p.z);
